#
# stmultitarget: cmake stm32 multi-target library
#
#  Ethan Zonca 2021
#
...
function(stmulti_stm32_info_from_startup_filestr IN_FILSTR)
  ...
  set(STMULTI_PROCESSOR_DEFINITION ${TARGET_PROCESSOR_DEFINITION} PARENT_SCOPE)
  # Target Processor Definition: all lowercase (e.g., stm32g471xx)
  set(STMULTI_PROCESSOR_DEFINITION_LOWERCASE ${DERIVED_TARGET_NAME} PARENT_SCOPE)
  # Startup.S filename (e.g., startup_stm32g471xx.s)
  set(STMULTI_STARTUP_FILENAME ${STARTUP_FILENAME} PARENT_SCOPE)
  # Target family (e.g., STM32F0)
  set(STMULTI_STARTUP_TARGET_FAMILY ${TARGET_FAMILY_UPPERCASE} PARENT_SCOPE)
...
function(stmulti_stm32_info_from_halconfig IN_FILSTR)
  ...
  set(STMULTI_HALCONF_PROCESSOR_SERIES ${PROCESSOR_SERIES} PARENT_SCOPE)
  # Target Project Name:  e.g. canable2
  set(STMULTI_HALCONF_PROJECT_NAME ${PROJECT_NAME} PARENT_SCOPE)
...
function(stmulti_add_hal_library PROCESSOR_FAMILY CORE_TYPE SOURCES_IN INCLUDES_IN)
    ...
    foreach(halconfig ${halconfigs})
        ...
        #foreach(startupfile ${startupfiles})
            ...
            stmulti_stm32_info_from_halconfig(${halconfig})
            stmulti_stm32_info_from_startup_filestr("startup_${STMULTI_HALCONF_PROCESSOR_SERIES}.s")
            ...
            message("Comparing ${STMULTI_STARTUP_TARGET_FAMILY} ${PROCESSOR_FAMILY}")
            
            if(STMULTI_STARTUP_TARGET_FAMILY STREQUAL PROCESSOR_FAMILY)
                message("Building target ${STMULTI_HALCONF_PROCESSOR_SERIES} for project ${STMULTI_HALCONF_PROJECT_NAME}")

                string(CONCAT MODULE_NAME "HAL_" ${STMULTI_HALCONF_PROCESSOR_SERIES} "_" ${STMULTI_HALCONF_PROJECT_NAME})
                ...
                target_compile_definitions(${MODULE_NAME} PRIVATE ${STMULTI_PROCESSOR_DEFINITION})
                
                # This sets which HAL conf file we include
                target_compile_definitions(${MODULE_NAME} PRIVATE -DTARGET_HAL_CONFIGURATION="${STMULTI_HALCONF_PROJECT_NAME}_${STMULTI_HALCONF_PROCESSOR_SERIES}_hal_conf.h")
                target_sources(${MODULE_NAME} PRIVATE "../../targets/${STMULTI_HALCONF_PROJECT_NAME}_${STMULTI_HALCONF_PROCESSOR_SERIES}_hal_conf.h")
                
                #target_sources(${MODULE_NAME} PRIVATE "cmsis/src/startup_${STMULTI_HALCONF_PROCESSOR_SERIES}.s")

                
                message("Adding source ../../targets/${STMULTI_HALCONF_PROJECT_NAME}_${STMULTI_HALCONF_PROCESSOR_SERIES}_hal_conf.h")
                
                #target_sources(${MODULE_NAME} PRIVATE cmsis/src/${STMULTI_STARTUP_FILENAME})
                target_sources(${MODULE_NAME} PRIVATE ../../targets/${STMULTI_HALCONF_PROJECT_NAME}_${STMULTI_STARTUP_FILENAME})
          
          #message("Added target defintion ${STMULTI_PROCESSOR_DEFINITION}")
